...
There is no multi-agent build pipeline — the earlier `.claude/agents/`, hooks, and Playwright/E2E test scaffolding have been removed. Tasks are implemented directly in-session, one at a time, following the same conventions those agents used to enforce (see `wp-plugin-dev`/`wp-plugin-review` under `.claude/skills/`). Mark a task `built` in `ServiceCrew-Tasks.md` once it's implemented; the user then checks it manually and marks it `done` themselves — don't mark a task `done` without that manual check.
